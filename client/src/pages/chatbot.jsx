...
export default function AskHRChat({ isFloating = false }) {
  ...
  const handleSubmit = async (customInput = null) => {
    ...
    try {
      ...
      // Decide rendering based on data size: small => explanation, large => table
      const isSmall = Array.isArray(data) && data.length <= 4;
      const followUps = getFollowUps(question);
      const assistantMessage = {
        role: 'assistant',
        content: isSmall ? explanation : data,
        isTable: !isSmall,
        followUps
      };
    ...
    return (
      <div className="table-container">
        <table className="data-table">
          <thead>
            <tr>
              {headers.map((key, i) => <th key={i}>{key}</th>)}
            </tr>
          </thead>
          <tbody>
            {data.map((row, ridx) => (
              <tr key={ridx}>
                {headers.map((key, cidx) => (
                  <td key={cidx}>
        ...
        <div className="export-buttons">
          <button onClick={() => exportToExcel(data)} className="export-button">📊 Export to Excel</button>
          <button onClick={() => exportToPDF(data)} className="export-button">📄 Export to PDF</button>
        </div>
      </div>
    );
  };
  ...
  const exportToExcel = (data) => {
    const capitalized = data.map(r => Object.fromEntries(Object.entries(r).map(([k,v]) => [k.toUpperCase(), v])));
    const ws = XLSX.utils.json_to_sheet(capitalized);
    ws['!cols'] = Object.keys(capitalized[0]).map(() => ({ wch: 20 }));
    Object.keys(capitalized[0]).forEach((h, i) => {
      const c = XLSX.utils.encode_cell({ r: 0, c: i });
      if (!ws[c]) return;
      ws[c].v = h;
      ws[c].s = { font: { bold: true }, fill: { fgColor: { rgb: 'D6EAF8' } }, alignment: { horizontal: 'center' }, border: { top: { style: 'thin', color: { rgb: 'BBBBBB' } }, bottom: { style: 'thin', color: { rgb: 'BBBBBB' } }, left: { style: 'thin', color: { rgb: 'BBBBBB' } }, right: { style: 'thin', color: { rgb: 'BBBBBB' } } } };
    });
    const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, ws, 'Table Data');
  ...
  const exportToPDF = (data) => {
    ...
    const body = data.map(r => hdr.map(h => r[h]));
    autoTable(doc, { head: [hdr], body, startY: 10 });
      ...
      <div className="messages-container">
        {messages.length === 0 ? (
          <div className="empty-state">
            ...
            <p>Ask about company policies, benefits, leave, or HR questions</p>
            <div className="suggestions">
              {suggestions.map((s, i) => (
                <button key={i} className="suggestion-button" onClick={() => { setInput(s); handleSubmit(s); }}>{s}</button>
              ))}
            </div>
          </div>
        ) : (
          messages.map((msg, idx) => (
            <div key={idx} className={`message ${msg.role==='user'?'user-message':'assistant-message'}`}>
              ...
              <div className="message-content">
                ...
                {!msg.isTable && <div className="message-body">{msg.content}</div>}
                {msg.isTable && <div className="message-body">{renderTable(msg.content)}</div>}
                {msg.followUps && msg.followUps.length>0 && (
                  <div className="follow-up-questions">
                    {msg.followUps.map((q,i)=>(<button key={i} className="follow-up-button" onClick={()=>{ setInput(q); handleSubmit(q); }}>{q}</button>))}
                  </div>
                )}
              </div>
            </div>
